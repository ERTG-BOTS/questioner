...
public static class Substitution
{
  ...
  public static readonly Dictionary<string, short> ModeCode = new()
    ...
    public static async Task DelayToTime(TimeOnly timeTo)
    {
        ...
        var timeToDelay = new DateTime(timeFromDelay.Year, timeFromDelay.Month, timeFromDelay.Day, timeTo.Hour,
            timeTo.Minute, 0);
    ...
    public static void WriteLog(string mode, string output)
    {
        Console.WriteLine($"{DateTime.UtcNow.AddHours(5):d.MM.yyyy | HH:mm:ss} {ConsoleLog[mode]} {output}");
    }

    public static string WriteOutFormattedLog()
    {
        StringBuilder output = new();
        foreach (var kvp in ConsoleLog) output.AppendLine($"{kvp.Key} | {kvp.Value}");
    ...
    public static string ConvertDate(string inputDate)
    {
        ...
        var date = DateTime.ParseExact(dateString, "dd.MM.yyyy", russianCulture);
        date = date.AddDays(-1);
