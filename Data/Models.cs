...
public class RegisteredUsersModel
{
  ...
  [Required]
  public required string FIO { get; set; } // Представляет полное имя пользователя.
  ...
  public bool MatchFIO(string fio)
  {
    ...
    for (int i = 1; i < fioMatch.Groups.Count; i++)
      if (!FIOMatch.Groups[i].Value.Contains(fioMatch.Groups[i].Value, StringComparison.OrdinalIgnoreCase)) return false;
...
public class UserModel
{
  ...
  public static UserModel? GetUser(long chatId, string username = "")
  {
    ...
    using (AppDbContext db = new())
    {
      ...
      if ((username == "Скрыто/не определено" && userByChatId != null) || userByUsername == userByChatId) { user = userByChatId; }
  ...
  public bool ModeCodeKey(string partialKey)
  {
    foreach (var keyValuePair in Substitution.ModeCode)
    {
      ...
      {
        string modeCodeKey = keyValuePair.Key.ToLower();
