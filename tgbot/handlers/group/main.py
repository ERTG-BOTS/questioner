...
Оцени, мог ли специалист решить его самостоятельно""",
            ...
            await message.bot.send_message(chat_id=employee.ChatId, text="<b>🔒 Вопрос закрыт</b>",
                                           reply_markup=ReplyKeyboardRemove())

            await message.bot.send_message(chat_id=employee.ChatId, text=f"""Старший <b>{duty.FIO}</b> закрыл вопрос
...
async def dialog_quality_duty(callback: CallbackQuery, callback_data: QuestionQualityDuty, stp_db):
    ...
    if dialog.TopicDutyFullname == duty.FIO:
        await repo.dialogs.update_question_quality(token=callback_data.token, quality=callback_data.answer,
                                                   is_duty=True)
