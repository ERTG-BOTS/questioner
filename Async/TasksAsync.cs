...
public class TasksAsync
{
    ...
    public static async Task ExpirationOverwatchTask()
    {
        ...
        foreach (var dialog in QueueManager.DialogChats.Where(x =>
                     ...
                     utcNow.Subtract((DateTime)x.LastMessageReceived).TotalMinutes > 5))
            if (utcNow.Subtract((DateTime)dialog.LastMessageReceived!).TotalMinutes > 10)
            {
                await botClient.SendMessage(
                    dialog.ChatIdEmployee,
                    "<b>🔒 Отсутствие активности | Закрытие</b>\n\nЧат был неактивен в течение 10 минут и сейчас будет закрыт",
                    parseMode: ParseMode.Html
                );
            ...
            else
            {
                await botClient.SendMessage(
                    dialog.ChatIdEmployee,
                    "<b>🚩 Отсутствие активности | Предупреждение</b>\n\nЧат был неактивен в течение 5 минут. Он закроется через 5 минут если не будет активности" +
