...
public class DocumentAsync
{
    public static async Task DialogHistoryExcel(long chatId, int month)
    {
        using (AppDbContext dbContext = new())
        {
            var dialogHistory = dbContext.DialogHistory
                .ToList()
                .Where(x => int.TryParse(x.StartQuestion.Split('.')[1], out var Month) && Month == month)
            ...
            using (var package = new ExcelPackage())
            {
                ...
                foreach (var dialog in dialogHistory)
                {
                    ...
                    worksheet.Cells[row, 7].Value =
                        dialog.DialogQuality switch { true => "Хорошо", false => "Плохо", _ => "Нет оценки" };
                    worksheet.Cells[row, 8].Value =
                        dialog.DialogQualityRg switch { true => "Хорошо", false => "Плохо", _ => "Нет оценки" };
                    foreach (var item in dialog.ListFIOSupervisor.Split(";"))
                        worksheet.Cells[row + rowBuffer++, 3].Value = item;
                    if (rowBufferMax < rowBuffer) rowBufferMax = rowBuffer;
                    rowBuffer = 0;
    ...
    public static async Task DialogHistoryExcel(long chatId)
    {
        ...
        using (AppDbContext dbContext = new())
        {
            var dialogHistory = dbContext.DialogHistory.ToList()
                .Where(x => DateTime.ParseExact(x.StartQuestion, "dd.MM.yyyy HH:mm:ss", russianCulture)
                    .CompareTo(DateTime.UtcNow.AddMonths(-3)) > 0)
                .OrderBy(x => x.FirstMessageId)
                .ToList();
        ...
        using (AppDbContext dbContext = new())
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                for (var i = 0; i < dialogHistory.Count; i++)
                {
                    ...
                    worksheet.Cells[i + 2, 4].Value = record.TimeDialogStart.AddHours(3)
                        .ToString("dd.MM.yyyy HH:mm:ss", russianCulture);
                    worksheet.Cells[i + 2, 5].Value = record.TimeDialogEnd.AddHours(3)
                        .ToString("dd.MM.yyyy HH:mm:ss", russianCulture);
    ...
    {
        var dialogHistory = currentDialog.DialogHistory.Split('@');
        if (currentDialog.FIOSupervisor == currentDialog.FIOEmployee)
        {
            var debugHistory = dialogHistory.Select(x => x.Split('#')[0]).Where(x => !string.IsNullOrEmpty(x))
                .Distinct().ToList();
            if (debugHistory.Count == 2 && int.TryParse(debugHistory[0], out var chatId0) &&
                int.TryParse(debugHistory[1], out var chatId1))
        ...
        var fileName = $"Диалог_{currentDialog.TokenDialog}.pdf";

        var fontPath = Path.Combine(AppContext.BaseDirectory, "Data", "TimesNewRoman.ttf");
        var filePath = Path.Combine($"{AppContext.BaseDirectory}", "buffer", "dialog.pdf");
        var dialogDocument = Path.Combine($"{AppContext.BaseDirectory}", "buffer", "dialogDocument");
        var messagePhotoPath = Path.Combine($"{AppContext.BaseDirectory}", "buffer", "dialogPhoto.jpg");

        using PdfWriter writer = new(filePath);

        var font = PdfFontFactory.CreateFont(fontPath, "cp1251", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED,
            true);
        ...
        using (AppDbContext db = new())
        {
            var a = db.RegisteredUsers
                .Where(x => x.FIO == currentDialog.FIOEmployee || x.FIO == currentDialog.FIOSupervisor).ToList();
            if (a.Count == 2)
            {
        ...
        try
        {
            ...
            var start = string.Format("Диалог {0} с {1} в {2} - {3} МСК\n",
                currentDialog.FIOSupervisor,
                currentDialog.FIOEmployee,
                currentDialog.TimeDialogStart.AddHours(3).ToString("dd.MM.yyyy HH:mm:ss", russianCulture),
                currentDialog.TimeDialogEnd.AddHours(3).ToString("HH:mm:ss", russianCulture));
            ...
            foreach (var message in dialogHistory)
            {
                if (message == "") break;
                var messageSplit = message.Split('#');
                var currentChatId = long.Parse(messageSplit[0]);
                var thisMessageId = await botClient.CopyMessage(
                    5405986946,
                    currentChatId,
                    int.Parse(messageSplit[1])
                );
                var thisMessage = await botClient.EditMessageReplyMarkup(
                    5405986946,
                    thisMessageId.Id,
                    new InlineKeyboardMarkup(InlineKeyboardButton.WithCallbackData("1"))
                );

                var sender = currentChatId == usersInDialog[0]
                    ? currentDialog.FIOEmployee
                ...
                var textMessage = thisMessage.Caption ?? thisMessage.Text ?? null;
                if (textMessage != null)
                ...
                {
                    var messagePhoto = await botClient.GetFile(
                        thisMessage.Photo.Last().FileId
                    );
                    var downloadPhoto = new FileStream(messagePhotoPath, FileMode.Create);
                    await botClient.DownloadFile(messagePhoto!.FilePath!, downloadPhoto);
                    downloadPhoto.Dispose();
                    var imagedata = ImageDataFactory.Create(messagePhotoPath);
                ...
                if (thisMessage.Document != null)
                {
                    ...
                    {
                        InputFile inputFileFromStream = InputFile.FromStream(fileStream,
                            $"Документ_{documentNumber++}_{thisMessage.Document.FileName}");
                        await botClient.SendDocument(
                            chatId,
                            inputFileFromStream
                        );
